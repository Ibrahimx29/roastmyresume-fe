  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center">
      ...
      <p className="text-xl text-gray-300 mb-8 max-w-2xl">
        Upload your resume and get a brutally honest, yet strangely helpful
        critique. It might hurt, but so does rejection emails.
      </p>
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
        {[
          { name: "A Developer", quote: "My resume cried. 10/10." },
          {
            name: "HR Manager",
            quote:
              "Savage but accurate. I hired the person after they fixed everything.",
          },
          {
            name: "Recent Grad",
            quote: "I didn't know my resume was that bad. Now I do.",
          },
        ].map((testimonial, i) => (
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh]">
      ...
      <button
        ...
        className={`flex items-center bg-red-600 hover:bg-red-700 disabled:bg-gray-600 text-white font-bold py-3 px-6 rounded-xl transition-all ${
  ...
  const formatResumeText = (text) => {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      if (
        lines[i].includes("@") ||
        lines[i].includes("+") ||
        lines[i].match(/\b[A-Z][a-z]+\s[A-Z][a-z]+\b/)
      ) {
            ...
            <div className="space-y-1">
              {section.content.split("\n").map((paragraph, pIndex) => {
                ...
                if (
                  ...
                  (paragraph.match(/^[A-Za-z]/) && paragraph.endsWith("."))
                ) {
                  return (
        ...
        {contactInfo && (
          <div className="mt-6 pt-4 border-t border-gray-600 text-center">
            {contactInfo.split("\n").map((line, i) => (
              <div
                key={`contact-${i}`}
                className={i === 0 ? "font-bold text-lg" : "text-gray-300"}
              >
                {line}
              </div>
            ))}
